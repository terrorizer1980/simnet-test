...
async function setupRoger() {
  ...
  const w = Wallet.fromMnemonic(
    "program biology gasp gentle describe boring suspect raven favorite uphold salon crater"
  ).connect(provider2);
...
const getRandomEntity = async function (
  entityArr: RestServerNodeService[],
  num_entities: number
) {
  const randEntities = entityArr
...
const setupChannelsBetweenNodesAndRouter = async function (
  nodes: RestServerNodeService[],
  router: RestServerNodeService
) {
...
async function start() {
  ...
  const randomRouter: RestServerNodeService = await getRandomEntity(
    routers,
    1
  )[0];
  const randomNodes: RestServerNodeService[] = await getRandomEntity(nodes, 3);

  const res = await setupChannelsBetweenNodesAndRouter(
    randomNodes,
    randomRouter
  );
...
export const dynamicDeposit = async (
  ...
  const chainId = await provider.getNetwork().then((p) => {
    return p.chainId;
  });

  //get depositor balance of asset
  const beforeDepositBalance = (channel: FullChannelState, assetId: string) => {
    const assetIdx = channel.assetIds.findIndex(
      (a) => getAddress(a) === getAddress(assetId)
    );
    if (assetIdx === -1) {
      return "0";
    }
...
const main = async function () {
  ...
  console.log(
    "Transfer Result Context",
    transferRes.getError()?.context.context
  );
